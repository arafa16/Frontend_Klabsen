...
export const createPrivileges : any = createAsyncThunk("createPrivileges", async(privileges : any, thunkAPI) => {
    try {
        const response = await axios.post(import.meta.env.VITE_REACT_APP_API_URL+`/privileges`,{
            ...
            userSub:privileges.userSub,
            eventSub:privileges.eventSub,
            koreksiAdminSub:privileges.koreksiAdminSub,
            perhitunganNilaiSub:privileges.perhitunganNilaiSub,
            etiket:privileges.etiket,
            pengajuanKendalaSub:privileges.pengajuanKendalaSub,
